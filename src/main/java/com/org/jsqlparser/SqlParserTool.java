...
public class SqlParserTool {
    ...
    public static void main(String[] args) throws JSQLParserException {
        String sql1 = "select * from (select userid from a) a";

        String sql = "SELECT po_id,po_line_id\n" +
                "FROM withholding_base\n" +
                "WHERE id IN\n" +
                "    (SELECT base_id\n" +
                "     FROM operate_log\n" +
                "     WHERE operate_type = 700\n" +
                "       AND create_date > \"2020-09-27\")";
        ...
        if(sqlType.equals(SqlType.SELECT)){
            ...
            List<SelectItem> selectItems = SqlParserTool.getSelectItems(statement.getSelectBody());
            ...
            for (SelectItem selectItem : selectItems) {
                ...
                String str = selectItem.toString();
                System.out.println(str);
            }

            CommonObject commonObject = new CommonObject();
            List<CommonObjectSon> commonObjectSons = new ArrayList<>();

            for (SelectItem selectItem : selectItems) {
                CommonObjectSon commonObjectSon = new CommonObjectSon();
                commonObjectSon.setObj(selectItem.toString());
                commonObjectSons.add(commonObjectSon);
            }
            commonObject.setCommonObjectSons(commonObjectSons);
